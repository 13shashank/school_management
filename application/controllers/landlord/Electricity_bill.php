...
class Electricity_bill extends CI_Controller {
   public function __construct() {
    ...
    public function index(){
      $data=[];
      $data['script']="electricity_bill/script"; 
     
      $data['content']="electricity_bill/index";
    ...
    public function list(){
        $this->load->model('landlord/electricity_bill_model');
      if($this->input->server('REQUEST_METHOD')=='POST'){
       ...
       $data['count']=$this->electricity_bill_model->CountEbillList($keyword);
         $data['recordsFiltered']=$data['count'];
         $data['aaData']=$this->electricity_bill_model->EbillList(10,$offset,$keyword,$orderkey,$ordervalue);
        ...
        if($user_id == null){
           ...
           if($save){
            $message =  get_alert_tpl('Data Saved Successfully');
               ...
               redirect('landlord/electricity_bill');
            
           }
        }
        else{
          
            $update = $this->common_model->UpdateData('electricity_bill',$saverow,array('t_id'=>$user_id));
    ...
    $data = [];
    if($user_id !=null){
        $data['row'] = $this->common_model->GetRow('electricity_bill',array('t_id'=>$user_id));
       }
    
       $data['tenant'] = $this->common_model->GetResult('tenant');
      $data['content']="electricity_bill/edit";
      $this->load->view('landlord_template',$data);
    }
}